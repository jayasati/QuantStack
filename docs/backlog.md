...
- **CI / automated checks — removed a SECOND time, 2026-07-15, do not re-add without reading this first.** GitHub Actions workflows were originally removed by choice in `fee0703` (account billing lock; owner opted for plain git version control). On 2026-07-11 (`edcfabf`) a workflow was re-added to address an audit's "no CI/CD" finding, without checking this note first — the billing lock was never lifted, so every run silently failed to even acquire a runner (0/5 runs executed a single step, each completing in <1s) for 4 days before anyone noticed, via `docs/volumes/postflight-vol1-2026-07-15.md`. Re-removed the same day. **This is now the permanent policy, not a temporary gap**: this is a solo-contributor project, all testing happens locally and on quantstack-vm, and CI adds no protection this workflow doesn't already provide. The last CI workflow version (ruff, mypy, migration up/down/up, pytest against live Postgres/Redis, Docker build) and the Pages deploy workflow live in git history at commit `02275cc` if ever revisited — but check whether the billing lock is actually lifted before doing so, don't just restore the file. Until then: full suite (`cd backend && python -m pytest app/tests -q`, ~5 min, ~1185 tests) runs locally before every push per `prompts/2-build.md` / `prompts/VERIFY-COOKBOOK.md`; `ruff check app` / `mypy app` locally too; docs publish via `mkdocs gh-deploy --force --no-history`.
